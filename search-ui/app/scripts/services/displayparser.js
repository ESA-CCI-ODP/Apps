...
/**
 * @ngdoc service
 * @name searchUiApp.displayParser
 * @description
 * # displayParser
  ...
  .service('displayParser', function () {
    ...
    api.parseCWSresultToDisplay = function (cswResults) {
      ...
      _.each(cswResults, function (res) {
        ...
        disObject.bbox = extractGeographicExtent(res);
    ...
    function extractGeographicExtent(result, position) {
      if (!_.isUndefined(result.distributionInfo)) {
        var extent = result.identificationInfo.MD_DataIdentification.extent;
        if (!_.isUndefined(extent[0])) { //getting temportal array
          return (extent[0].EX_Extent.geographicElement.EX_GeographicBoundingBox);
        }
      }
    }
